...
L'aspect intéressant de ce classement est notamment d'inclure les caractères spéciaux et de ponctuation que l'on trouve dans la plupart des textes. Ainsi, nous avons un classement complet sur lequel baser et organiser la recherche corrélative sur chacune des tuiles. Cette méthode est notamment associée à l'\hyperref[stop_cor]{\textcolor{blue}{arrêt de recherche corrélative}}, qui permet alors de passer plus rapidement aux caractères suivants lorsque le caractère semblant être détecté est un caractère couramment utilisé.
